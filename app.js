...
/**
 * Upload Image Helper
 * Uploads image directly to Firebase Storage if Firebase is enabled,
 * or converts image to compressed Base64 Data URL as a reliable fallback.
 */
async function uploadImageToServer(file) {
  ...
  if (file.size > 15 * 1024 * 1024) {
    showToast('ไฟล์รูปภาพมีขนาดใหญ่เกินไป (สูงสุด 15MB)');
    return null;
  }

  // Option 1: Firebase Storage (if enabled)
  if (isFirebaseEnabled && storage) {
    try {
      showToast('กำลังอัปโหลดรูปภาพไปยัง Firebase Storage...');
      const cleanName = file.name.replace(/[^a-zA-Z0-9._-]/g, '_');
      const fileName = `images/img_${Date.now()}_${Math.floor(Math.random() * 10000)}_${cleanName}`;
      ...
      showToast('ไม่สามารถอัปโหลดไปยัง Firebase Storage ได้ (กำลังใช้โหมดสำรอง Base64)');
    }
  }

  // Option 2: Local File Reader -> Compressed Base64 Data URL (Fallback)
  return new Promise((resolve) => {
    const reader = new FileReader();
    reader.onload = (evt) => {
      const base64Data = evt.target.result;
      showToast('อัปโหลดรูปภาพสำรองเรียบร้อย');
      resolve(base64Data);
    };
    reader.onerror = () => {
      showToast('เกิดข้อผิดพลาดในการอ่านไฟล์รูปภาพ');
      resolve(null);
    };
