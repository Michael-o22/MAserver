...
function sanitizeInventoryData() {
  ...
  state.inventory.forEach(rack => {
    ...
    rack.items.forEach((item, idx) => {
      ...
      // Fix duplicate 5.1 in Rack 5 (SmartPower UPS was mistakenly numbered 5.1 instead of 5.10)
      if (item.item_no === '5.1') {
        count51++;
        if (count51 > 1 || (item.name_description && (item.name_description.includes('SmartPower') || item.name_description.includes('6 KVA')))) {
          item.item_no = '5.10';
          hasChanges = true;
        }
      } else if (item.name_description && (item.name_description.includes('SmartPower') || item.name_description.includes('6 KVA')) && item.item_no !== '5.10') {
        item.item_no = '5.10';
        hasChanges = true;
      }
      ...
      if ((rack.rack_id === 'rack_4' || rack.rack_no === '4') && item.item_no === '4.7' && rack.items.length === 6) {
